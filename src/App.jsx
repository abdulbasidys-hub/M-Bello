            ...
            <div style={{background:'linear-gradient(135deg,#78350F,#D97706)',padding:'20px 20px 16px',borderRadius:'22px 22px 0 0'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <div style={{width:38,height:38,borderRadius:10,background:'rgba(255,255,255,0.15)',display:'flex',alignItems:'center',justifyContent:'center'}}><UserPlus size={17} color="#fff"/></div>
                  <div>
                    <div style={{fontFamily:"'Outfit',sans-serif",fontSize:16,fontWeight:800,color:'#fff',lineHeight:1}}>Add Member</div>
                    <div style={{fontSize:11,color:'#FDE68A',marginTop:2}}>Syncs instantly</div>
                  </div>
                </div>
                <button onClick={()=>{setAddModal(false);setAddName('');}} style={{width:28,height:28,borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,0.15)',border:'none',cursor:'pointer',color:'#fff'}}><X size={14}/></button>
              </div>
            </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',backdropFilter:'blur(8px)',zIndex:60,display:'flex',alignItems:'flex-end',justifyContent:'center'}} onClick={e=>e.target===e.currentTarget&&setEditModal(null)}>
          <div className="su" style={{background:t.surface,borderRadius:'22px 22px 0 0',width:'100%',maxWidth:520,boxShadow:t.shLg}}>
            <div style={{background:'linear-gradient(135deg,#0F3460,#1A6BAE)',padding:'20px 20px 16px',borderRadius:'22px 22px 0 0'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div style={{display:'flex',alignItems:'center',gap:10}}>
                  <div style={{width:38,height:38,borderRadius:10,background:'rgba(255,255,255,0.15)',display:'flex',alignItems:'center',justifyContent:'center'}}><Edit3 size={17} color="#fff"/></div>
                  <div>
                    <div style={{fontFamily:"'Outfit',sans-serif",fontSize:16,fontWeight:800,color:'#fff',lineHeight:1}}>Edit Name</div>
                    <div style={{fontSize:11,color:'#BAE6FD',marginTop:2}}>Updates everywhere</div>
                  </div>
                </div>
                <button onClick={()=>setEditModal(null)} style={{width:28,height:28,borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,0.15)',border:'none',cursor:'pointer',color:'#fff'}}><X size={14}/></button>
              </div>
            </div>
            <div style={{padding:'16px 20px 36px'}}>
      ...
      <main style={{maxWidth:1300,margin:'0 auto',padding:'16px 14px 120px',width:'100%'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(2,1fr)',gap:10,marginBottom:18}}>
          ...
          <div className="ch" style={{background:t.surface,border:`1px solid ${t.border}`,borderRadius:16,padding:'13px 14px',boxShadow:t.sh}}>
            ...
            <div style={{fontFamily:"'Outfit',sans-serif",fontSize:16,fontWeight:800,color:t.text,letterSpacing:'-0.5px',lineHeight:1}}>
              {stats.paid}<span style={{fontSize:12,color:t.textMuted,fontWeight:600}}>/{stats.total}</span>
            </div>
        ...
        {view==='master' && (
          <div className="fi" style={{background:t.surface,borderRadius:16,border:`1px solid ${t.border}`,boxShadow:t.sh,overflowX:'auto',WebkitOverflowScrolling:'touch'}}>
            ...
            <table style={{width:'100%',borderCollapse:'collapse',minWidth:840}}>
              <thead>
                <tr>
                  <th style={{padding:'11px 14px',fontSize:10,fontWeight:700,color:t.textMuted,textTransform:'uppercase',letterSpacing:'0.08em',textAlign:'left',borderBottom:`1px solid ${t.border}`,background:t.surfaceAlt,position:'sticky',left:0,zIndex:2,whiteSpace:'nowrap',minWidth:130}}>Name</th>
                  {MONTHS.map(m=>(
                    <th key={m} style={{padding:'11px 9px',fontSize:10,fontWeight:700,color:m===month?t.accent:t.textMuted,textTransform:'uppercase',letterSpacing:'0.08em',textAlign:'right',borderBottom:`1px solid ${t.border}`,background:m===month?`${t.accent}10`:t.surfaceAlt,whiteSpace:'nowrap',minWidth:48}}>
                      {m.slice(0,3)}
                    </th>
                  ))}
                  <th style={{padding:'11px 12px',fontSize:10,fontWeight:700,color:t.accentText,textTransform:'uppercase',letterSpacing:'0.08em',textAlign:'right',borderBottom:`1px solid ${t.border}`,background:`${t.accent}0E`,position:'sticky',right:0,zIndex:2,whiteSpace:'nowrap',minWidth:72}}>Total</th>
                </tr>
              </thead>
              <tbody>
      ...
      <div style={{position:'fixed',bottom:16,left:'50%',transform:'translateX(-50%)',zIndex:50,background:dark?'#1C2333EE':'#0F172AEE',backdropFilter:'blur(16px)',border:'1px solid rgba(255,255,255,0.08)',borderRadius:99,padding:'4px 6px',display:'flex',alignItems:'center',gap:2,boxShadow:'0 8px 32px rgba(0,0,0,0.45)'}}>
        ...
        {role==='superadmin' && (
          <>
            ...
            <button onClick={()=>setManageView(true)} className="bp" style={{width:42,height:42,borderRadius:99,display:'flex',alignItems:'center',justifyContent:'center',background:'transparent',color:'#F59E0B',border:'none',cursor:'pointer',transition:'all .15s'}}>
              <Settings size={17}/>
            </button>
          </>
        )}
        <div style={{width:1,height:20,background:'rgba(255,255,255,0.12)',margin:'0 2px'}}/>
        <button onClick={()=>setShowLogin(true)} className="bp" style={{width:42,height:42,borderRadius:99,display:'flex',alignItems:'center',justifyContent:'center',background:'transparent',color:role?(role==='superadmin'?'#F59E0B':'#10B981'):'#94A3B8',border:'none',cursor:'pointer',transition:'all .15s'}}>
            ...
            <div style={{background:'linear-gradient(135deg,#1E1B4B,#4338CA)',padding:'20px 20px 16px',borderRadius:'22px 22px 0 0'}}>
              <div style={{display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                <div style={{display:'flex',alignItems:'center',gap:11}}>
                  <div style={{width:40,height:40,borderRadius:11,background:'rgba(255,255,255,0.13)',display:'flex',alignItems:'center',justifyContent:'center'}}><Unlock size={18} color="#fff"/></div>
                  <div>
                    <div style={{fontFamily:"'Outfit',sans-serif",fontSize:17,fontWeight:800,color:'#fff',lineHeight:1}}>Admin Gate</div>
                    <div style={{fontSize:11,color:'#A5B4FC',marginTop:3}}>Unlock editing features</div>
                  </div>
                </div>
                <button onClick={()=>setShowLogin(false)} style={{width:28,height:28,borderRadius:8,display:'flex',alignItems:'center',justifyContent:'center',background:'rgba(255,255,255,0.15)',border:'none',cursor:'pointer',color:'#fff'}}><X size={14}/></button>
              </div>
            </div>
            <form onSubmit={handleLogin} style={{padding:'16px 20px 36px'}}>
